...
/**
 * Created by Huoyunren on 2015/12/3.
 */
public class LocationUtils {
     ...
     * @return 返回方向角s
     */
    public static float getBearing(Location lastLocation, Location newLocation) {
        float bearing = 0.0f;
        float[] results = new float[2];
        if (lastLocation != null && newLocation != null) {
            computeDistanceAndBearing(lastLocation.getLatitude(), lastLocation.getLongitude(),
                    newLocation.getLatitude(), newLocation.getLongitude(), results);
            bearing = results[1];
        }
        return bearing;
